...
- **Sistemi HUD Minimappa Radar, Inventario TAB, Oggetti Distruttibili & Fix Respawn Mob M:** Integrata la minimappa radar top-right (`SandboxMinimap.cs`), la finestra inventario modal apribile via `TAB` con scheda statistiche armi (`SandboxInventoryWindow.cs`), il componente vasi distruttibili con drop loot (`PrototypeDestructibleVase.cs`), la barra di ricarica sopra la testa (`SandboxReloadBar.cs`) ed il fix del respawn mob via tasto `M` (`PrototypeMobSpawnerToggle.cs`). Tutti i test superati.
